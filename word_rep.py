...
#Step 4: Build and Train the Skip-Gram Model
...
with graph.as_default():
    ...
    train_labels = tf.placeholder(tf.int32, shape=[bathc_size, 1])
    valid_dataset = tf.constant(valid_examples, dtype=tf.int32)

    #Operations and variables pinned to the CPU due to lack of GPU implementation
    ...
    #Construct the SGD optimizer with a learning rate of 1.0
...
with tf.Session(graph=graph) as session:
    ...
    for step in xrange(num_steps):
        ...
        if step % 2000 == 0:
            if step > 0:
                average_loss /= 2000
            #Average_loss is an estimate of the loss over the last 2000 batches
...
def plot_with_labels(low_dim_embs, labels, filename="tsne.png"):
    ...
    plt.figure(figsize(18,18)) #Figsize is in inches
